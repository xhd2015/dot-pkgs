## Expected
- Exit code 0.
- 2 entries: root with `(dead main)`, worktree with `(worktree)`.
- Both paths appear even though root does not exist on disk.
...
func Assert(t *testing.T, req *Request, resp *Response, err error) {
	...
	if !strings.Contains(rootLine, "(dead main)") {
		t.Fatalf("root line should contain (dead main), got: %s", rootLine)
	}
