## Expected
- Exit code 0.
- 2 entries: root with `(main)` marker, worktree with `(worktree)` marker.
- Each full path appears on its own line.
...
func Assert(t *testing.T, req *Request, resp *Response, err error) {
	...
	if !strings.Contains(rootLine, "(main)") {
		t.Fatalf("root line should contain (main) marker, got: %s", rootLine)
	}
	if !strings.Contains(wtLine, "(worktree)") {
		t.Fatalf("worktree line should contain (worktree) marker, got: %s", wtLine)
	}
}
```
