...
- Exit code 0.
- Stdout lists dep1 then dep2 (scan path-sorted order) each at `./external/mydepN-main-2026-06-30`, then a final `wrk 2 deps`.
- Both external paths exist as linked git worktrees.
- Consumer `go.mod` has a `replace` for `example.com/dep1` and for `example.com/dep2`, each pointing at its external path.
...
func Assert(t *testing.T, req *Request, resp *Response, err error) {
	...
	// Each external dep worktree is owned by its DEP repo, not the consumer.
	assertWorktreeListContains(t, allDepsDepMainRepo(req.WorkRoot, "mydep1"), wantDep1)
	assertWorktreeListContains(t, allDepsDepMainRepo(req.WorkRoot, "mydep2"), wantDep2)
