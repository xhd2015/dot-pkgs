...
- Consumer `go.mod` has a `replace` for `example.com/dep1` but NOT for `example.com/consumer` (self skipped).
...
func Assert(t *testing.T, req *Request, resp *Response, err error) {
	...
	if allDepsHasReplaceForModule(mod, "example.com/consumer", "") {
		t.Fatalf("go.mod should NOT replace consumer's own module example.com/consumer: %+v", mod.Replace)
	}
}
```
