// Package web serves the wrk React SPA (dot-pkgs-react apps/wrkweb) and mounts wrkserver.
//
...
// distFS holds the production SPA build (copied from react/dist by script/build-frontend).
//
//go:embed all:dist
var distFS embed.FS
...
type Options struct {
	WrkHome string
	Port    int
	// Dev proxies non-API traffic to the Vite server under react/.
	Dev bool
}
...
func findReactDir() (string, error) {
	...
	dir := wd
	for {
		candidate := filepath.Join(dir, "react")
	...
	return "", fmt.Errorf("wrk: could not find react/ package (set WRK_REACT_DIR or run from the monorepo)")
}
