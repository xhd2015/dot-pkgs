...
- No module whose `Dir` is `ignored` or starts with `ignored/` — the gitignored subtree
  is pruned.
...
func Assert(t *testing.T, req *Request, resp *Response, err error) {
	...
	for _, m := range resp.Modules {
		if m.Dir == "ignored" || strings.HasPrefix(m.Dir, "ignored/") {
			t.Fatalf("gitignored dir was not skipped: %+v", m)
		}
	}
}
```
