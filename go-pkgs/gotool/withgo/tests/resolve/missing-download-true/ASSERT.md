...
- `ResolveGoroot` returns the hook path.
- Prompt is written to Stderr before/as install runs.
...
func Assert(t *testing.T, d *session.Doctest, req *Request, resp *Response, err error) {
	...
	if resp.Goroot != req.HookGoroot {
		t.Fatalf("ResolveGoroot(%q) = %q, want hook path %q", req.GoVersion, resp.Goroot, req.HookGoroot)
	}
