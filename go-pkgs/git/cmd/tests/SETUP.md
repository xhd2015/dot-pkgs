...
- Package `github.com/xhd2015/dot-pkgs/go-pkgs/git/cmd` is importable.
- Real `git` on PATH for leaves that exercise subprocesses.

## Context

- Tests skip when `git` is unavailable.
- Helpers create minimal repos under `t.TempDir()`.
