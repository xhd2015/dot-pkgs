...
- Proves `Refresh=true` force-cold walk finds the post-seed plant that warm would miss.
...
- Cache remains enabled (`NoCache=false`); this leaf asserts discovery only.
...
func Assert(t *testing.T, d *session.Doctest, req *Request, resp *Response, err error) {
	...
	if !req.Refresh {
		t.Fatal("req.Refresh must be true for this leaf")
	}
	if len(resp.Repos) != 2 {
		t.Fatalf("expected 2 repos (force cold via Refresh), got %d", len(resp.Repos))
	}
