...
- The argv carries `--new-window`, so the request goes to a new window and the
  desktop does not switch Spaces to an existing one.
- The application resolves to the real bundle name `Opera`, not the lowercase
  alias that was typed.
