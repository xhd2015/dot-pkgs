...
func toSnapshotProc(p ProcRow, now time.Time) SnapshotProc {
...
func applyChosenToSession(s *SnapshotSession, idle *bool, shellPID *int, chosen *ProcRow, cwd *string, procs []SnapshotProc, now time.Time) {
	s.Idle = idle
	s.ShellPID = shellPID
	s.Cwd = cwd
	s.Processes = procs
